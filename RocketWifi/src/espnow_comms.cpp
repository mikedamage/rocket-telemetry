...
bool initESPNow(const uint8_t* groundStationMAC, CommandCallback callback) {
    ...
    Serial.printf("Ground station peer added: %02X:%02X:%02X:%02X:%02X:%02X\n",
                  groundStationAddress[0], groundStationAddress[1], groundStationAddress[2],
                  groundStationAddress[3], groundStationAddress[4], groundStationAddress[5]);

    return true;
}

bool sendTelemetry(const SensorReading& reading) {
    esp_err_t result = esp_now_send(groundStationAddress,
    ...
    if (result == ESP_OK) {
        ...
        // This prevents double-counting when send is initiated but delivery fails
...
uint32_t getTelemetrySentCount() {
    return telemetrySentCount;
}

uint32_t getTelemetryFailCount() {
    return telemetryFailCount;
}

uint32_t getCommandsReceivedCount() {
    return commandsReceivedCount;
}
